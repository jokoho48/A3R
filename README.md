...
A3R by {A3R}
```
Note that both lower and upper case are necessary. Once done, replace the logo (`logo_A3R_ca.paa`) by your own. Keep the same aspect ratio and file extension (`.paa`). This is important for good results in game. Also remember to rename the logo file to `logo_{your_prefix}_ca.paa`.
